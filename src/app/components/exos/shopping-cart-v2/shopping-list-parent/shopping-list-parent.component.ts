...
class CartEntry implements ICartEntry {
  itemID: number;
  itemName : string;
  amount: number;
  itemPrice: number;
...
class Cart implements ICart {
  ...
  AddOne(displayItem: IDisplayItem): void {
    ...
    if (alreadyIn) {
      ...
      this.itemList.push(new CartEntry(displayItem.itemName, displayItem.itemID, 1, displayItem.itemPrice))
    }
...
class DisplayItem implements IDisplayItem {
  itemID: number;
  imgUrl: string;
  itemPrice: number;
  itemName: string;
...
@Component({
  selector: 'app-shopping-list-parent',
  templateUrl: './shopping-list-parent.component.html',
  styleUrls: ['./shopping-list-parent.component.scss']
})
export class ShoppingListParentComponent {

  addToCartRequest(receivedId : number) :void {

    for (let i : number = 0; i < itemList.length; i++) {
      if (itemList[i].itemID === receivedId) {
        itemList[i].AddToCart(cart);
      }
    }
  ...
  protected readonly cart : ICart = cart;
}
